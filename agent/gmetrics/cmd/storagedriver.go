...
var (
	storageDriver   = flag.String("storage_driver", "", fmt.Sprintf("要使用的存储“驱动程序”。数据总是很快缓存在内存中，这控制了数据在本地缓存之外的推送位置。空表示无，多个用逗号分隔。选项为：<empty>, %s", strings.Join(storage.ListDrivers(), ", ")))
	storageDuration = flag.Duration("storage_duration", 2*time.Minute, "保存数据的时间（默认值：2分钟）。")
)
